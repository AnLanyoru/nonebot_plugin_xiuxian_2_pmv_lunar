...
async def sect_users_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if user_info:
        ...
        if sect_id:
            ...
            msg = f"☆【{sect_info['sect_name']}】的成员信息☆\r"
            msg_list.append(msg)
            userlist = userlist[user_num:user_num_end]
            i = user_num + 1
            for user in userlist:
                week_donate = limit_handle.get_user_donate_log_data(user['user_id'])
                msg = f"""编号{i}:{user['user_name']},{user['level']}\r宗门职位：{jsondata.sect_config_data()[f"{user['sect_position']}"]['title']}\r宗门贡献度：{number_to(user['sect_contribution'])}|{user['sect_contribution']}\r本周宗门贡献度：{number_to(week_donate)}|{week_donate}\r"""

                msg_list.append(msg)
                i += 1
            msg = f"\r第{page}/{page_all}页\r—————tips—————\r可以发送查看宗门成员+页码来查看更多宗门成员哦"
            msg_list.append(msg)
        else:
...
async def sect_users_donate_check_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if sect_id:
        ...
        userlist = [user for user in userlist if limit_handle.get_user_donate_log_data(user['user_id']) < goal_donate]
        page_all = ((len(userlist) // 12) + 1) if (len(userlist) % 12 != 0) else (len(userlist) // 12)  # 总页数
        ...
        if page_all < page:
            ...
            await sect_users_donate_check.finish()
        # 构造表头
        msg = f"☆【{sect_info['sect_name']}】本周贡献低于{number_to(goal_donate)}的成员信息☆\r"
        msg_list.append(msg)
        # 页数构造
        user_num = page * 12 - 12
        user_num_end = user_num + 12
        ...
        for user in userlist:
            ...
            msg = (f"编号{i}:{user['user_name']},{user['level']}\r"
                   f"宗门职位：{jsondata.sect_config_data()[str(user['sect_position'])]['title']}\r"
...
async def sect_task_complete_(bot: Bot, event: GroupMessageEvent):
    ...
    if sect_id:
        ...
        if userstask[user_id]['任务内容']['type'] == 1:  # type=1：需要扣气血，type=2：需要扣灵石
            ...
            await sql_message.update_user_sect_contribution(user_id, user_info['sect_contribution'] + int(sect_stone))
...
async def sect_kick_out_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    give_qq = await sql_message.get_user_id(args[0])  # 使用道号获取用户id，代替原at
...
async def my_sect_(bot: Bot, event: GroupMessageEvent):
    ...
    sect_info = await sql_message.get_sect_info(sect_id)
    owner_idx = [k for k, v in jsondata.sect_config_data().items() if v.get("title", "") == "宗主"]
    week_donate = limit_handle.get_user_donate_log_data(user_info['user_id'])
    owner_position = int(owner_idx[0]) if len(owner_idx) == 1 else 0
    if sect_id:
