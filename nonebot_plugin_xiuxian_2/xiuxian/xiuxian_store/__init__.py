...
@bind_break.handle(
    parameterless=[
        Cooldown(cd_time=24000, parallel_block=True)])
async def bind_break_(
    ...
    for item_type in ["技能", "装备"]:
        user_backs = await sql_message.get_back_goal_type_msg(user_id, item_type)  # list(back)
        if not user_backs:
            continue
...
@fast_sell_items.handle(
    parameterless=[
        Cooldown(cd_time=30, parallel_block=True)])
...
async def remove_want_item_(
    ...
    with user_cmd_lock:
        ...
        want_item_info = await user_store.check_user_want_item(user_id, item_id, 1)
...
async def user_sell_to_(
    ...
    async with store_sell_lock:
        ...
        if len(arg_strs) > 1:
            ...
            want_user_id = await sql_message.get_user_id(want_user_name)
            if not want_user_id:
                msg = f"修仙界中没有此人的踪迹！！！"
                await bot.send(event, msg)
                await user_sell_to.finish()
            ...
            want_user_info = await sql_message.get_user_info_with_id(want_user_id)
...
async def user_want_item_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    with user_cmd_lock:
        ...
        if item_price < min_price:
            ...
            await bot.send(event=event, message=msg)
            await user_want_item.finish()
        want_item = await user_store.check_user_want_item(user_id, item_id, 1)
        if want_item:
