...
async def Boss_fight(player1: dict, boss: dict, type_in=2, bot_id=0):
    ...
    if boss_buff.boss_zs > 0 :
        boss_zs_data = {"type": "node", "data": {"name": f"{boss['name']}",
                                     "uin": int(bot_id), "content": f"{boss['name']}使用了真龙九变,提升了{int(boss_buff.boss_zs *100)}%攻击力!"}}
        
        play_list.append(boss_zs_data)
        
    if boss_buff.boss_hx > 0  :
        boss_hx_data = {"type": "node", "data": {"name": f"{boss['name']}",
                                     "uin": int(bot_id), "content": f"{boss['name']}使用了无瑕七绝剑,提升了{int(boss_buff.boss_hx *100)}%会心率!"}}
        
        play_list.append(boss_hx_data)
    
    if boss_buff.boss_bs > 0 :
        boss_bs_data = {"type": "node", "data": {"name": f"{boss['name']}",
                                     "uin": int(bot_id), "content": f"{boss['name']}使用了太乙剑诀,提升了{int(boss_buff.boss_bs *100)}%会心伤害!"}}
        
        play_list.append(boss_bs_data)
        
    if boss_buff.boss_xx > 0 :
        boss_xx_data = {"type": "node", "data": {"name": f"{boss['name']}",
                                     "uin": int(bot_id), "content": f"{boss['name']}使用了七煞灭魂聚血杀阵,降低了{player1['道号']}{int((boss_buff.boss_xx) *100)}%气血吸取!"}}
        
        play_list.append(boss_xx_data)
        
    if boss_buff.boss_jg > 0 :
        boss_jg_data = {"type": "node", "data": {"name": f"{boss['name']}",
                                     "uin": int(bot_id), "content": f"{boss['name']}使用了子午安息香,降低了{player1['道号']}{int((boss_buff.boss_jg) *100)}%伤害!"}}
        
        play_list.append(boss_jg_data)
    
    if boss_buff.boss_jh > 0 :
        boss_jh_data = {"type": "node", "data": {"name": f"{boss['name']}",
                                     "uin": int(bot_id), "content": f"{boss['name']}使用了玄冥剑气,降低了{player1['道号']}{int((boss_buff.boss_jh) *100)}%会心率!"}}
        
        play_list.append(boss_jh_data)
        
    if boss_buff.boss_jb > 0 :
        boss_jb_data = {"type": "node", "data": {"name": f"{boss['name']}",
                                     "uin": int(bot_id), "content": f"{boss['name']}使用了大德琉璃金刚身,降低了{player1['道号']}{int((boss_buff.boss_jb) *100)}%会心伤害!"}}
    ...
    while True:
        ...
        ## 自己回合结束 处理 辅修功法14
        player1,boss,msg = after_atk_sub_buff_handle(player1_sub_open,player1,user1_main_buff_data,user1_sub_buff_date,
        ...
        if boss_turn_skip:
            ...
            if boss_sub <= 8:
                ...
                if isCrit:
                    ...
                    msg2 = "{}：紫玄掌！！紫星河！！！造成了{}伤害\n"
                play_list.append(get_boss_dict(boss, qx, msg2.format(boss['name'], boss_sh * (1 + boss_buff.boss_zs) * 5 + (player1['气血'] * 0.3)), bot_id))
                player1['气血'] = player1['气血'] - (((boss_sh * (1 + boss_buff.boss_zs) * (player1_js - random_buff.random_def) * 5) + (player1['气血'] * 0.3) ))
                play_list.append(get_boss_dict(boss, qx, f"{player1['道号']}剩余血量{player1['气血']}", bot_id))
                 ...
                 play_list.append(get_boss_dict(boss, qx, msg2.format(boss['name'], boss_sh * (1 + boss_buff.boss_zs) * (player1_js - random_buff.random_def + 0.5)  * 3), bot_id))
                 player1['气血'] = player1['气血'] - (((boss_sh * (1 + boss_buff.boss_zs) * (player1_js - random_buff.random_def + 0.5) * 3)))
                 play_list.append(get_boss_dict(boss, qx, f"{player1['道号']}剩余血量{player1['气血']}", bot_id))
                ...
                if isCrit:
                    ...
                    msg2 = "{}发起攻击，造成了{}伤害\n"
                play_list.append(get_boss_dict(boss, qx, msg2.format(boss['name'], boss_sh * (1 + boss_buff.boss_zs)), bot_id))
...
def get_turnatk(player, buff=0, user_battle_buff_date={},
    ...
    if random.randint(0, 100) <= player['会心'] + (impart_know_per + sub_crit - boss_buff.boss_jh + random_buff.random_hx )* 100:  # 会心判断
        turnatk = int(turnatk * (1.5 + impart_burst_per + weapon_critatk + main_critatk + sub_dmg - boss_buff.boss_jb)) #boss战、切磋、秘境战斗会心伤害公式（不包含抢劫）
        isCrit = True
