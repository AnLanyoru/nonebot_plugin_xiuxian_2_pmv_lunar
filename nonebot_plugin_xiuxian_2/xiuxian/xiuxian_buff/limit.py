...
class CheckLimit:
    ...
    async def check_send_stone_limit(self, user_id, stone_prepare_send):
        """
        检查送灵石数量是否超标
        :param user_id: 用户ID
    ...
    async def check_receive_stone_limit(self, user_id, stone_prepare_receive):
        """
        检查接收灵石是否达到上限
        :param user_id: 用户ID
    ...
    async def update_receive_stone_limit(self, user_id, new_date):
        """
        更新收灵石数量
        :param user_id: 用户ID
        :param new_date: 新数据
    ...
    async def update_send_stone_limit(self, user_id, new_date):
        """
        更新送灵石数量
        :param user_id: 用户ID
        :param new_date: 新数据
    ...
    async def send_stone_check(self, send_user_id, receive_user_id, num) -> tuple[str, str, bool]:
        ...
        """
        receive_user_info = await sql_message.get_user_info_with_id(receive_user_id)
        send_user_info = await sql_message.get_user_info_with_id(send_user_id)
        result_receive, is_receive_pass = await self.check_receive_stone_limit(receive_user_id, num)
